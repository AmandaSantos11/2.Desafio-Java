...
public class Vendedor extends Cadastro{
    ...
    static String senhaCriptografada;
    public Vendedor(String nome, String cpf, String email, String nomeDoProduto, double valorUnitarioDoProduto, String codigoDoProduto,String senha){
    ...
    static HashMap<String, String> gerarSenhaCriptografada = new HashMap<>();
    public void adicionarVendedoresNaLista(){
        listaDeVendedores.add(new Vendedor("João","12345678901","joao@gmail.com","Maça",2.50,"000","000000"));
        listaDeVendedores.add(new Vendedor("Maria","09876543211","maria.p@gmail.com","Chinelo",10.2,"001","222222"));
        for(Cadastro cadastro : listaDeVendedores){
            ...
            gerarSenhaCriptografada.put(email,senhaCriptografada);
        }
    }
    ...
    public void cadastrarNoSistema() throws UnsupportedOperationException{
        ...
        do {
            try{
                ...
                senha=digite.nextLine();
                if(senha.isBlank() || !(senha.matches("\\d+")) || senha.length()<6){
                    continuar=false;
                    throw new UnsupportedOperationException("Preenchimento inválido, digite novamente");
                }
                else{
        ...
        Vendedor vendedorNovo = new Vendedor(nome, cpf, email, nomeDoProduto, valorUnitarioDoProduto, codigoDoProduto,senha);
        gerarSenhaCriptografada.put(email,senhaCriptografada);
    ...
    public void login() throws UnsupportedOperationException {
        ...
        do{
            try {
                System.out.println("Digite sua senha:");
                senha = digite.nextLine();
                String senhaCripto = gerarSenhaCriptografada.get(email);
                if(BCrypt.checkpw(senha,senhaCripto)){
                    continuar=true;
                }
                else{
                    System.out.println("Preenchimento inválido, digite novamente");
                    continuar=false;
                }
            }catch (UnsupportedOperationException error) {
                System.err.println(error.getMessage());
